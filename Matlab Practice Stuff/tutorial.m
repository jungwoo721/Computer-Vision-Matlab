...
% Seong Jae Hwang <seongjae (AT) yonsei DOT ac DOT kr>, 09/03/2022
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Question: What can I do if I want to just debug a block of lines in my script, 
...
fprintf('The value of a in the script is: %s\n', num2str(a));
                             % Global variable a is unchanged
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% (d) Checking variables in workspace + Saving and closing your work

whos                         % See what variables (of what type and size) 
...
% using user-contributed content might be cheating depending on the assignment!
...
addpath 'C:\Users\seongjae\My Documents'
...
v = pi*[-4:4]/4              %   [start:stepsize:end]
                             %   (stepsize is 1 if not specified)
v = []                       % Empty vector
...
m(1, 1:3) = rand(1, 3)       % Replace those elements
...
who                          % Review: List variables in workspace
whos                         % Review: List variables w/ info about size, type, etc.
...
% Write down the answers to these before we try them!
...
mean(a, 1)                   % Same thing (second argument specifies
...
r1 == r2                     % Element-wise result        
all(r1 == r2)                % ... not quite. Why? 

all(all(r1 == r2))           % Ok
all(r1(:) == r2(:))          % Another way
...
find(indicator)              % Return indices where true
...
b = [3 4]
c = [a b]                    % Horizontal concatenation (also see horzcat)

a = [1; 2; 3];               % Column vector

c = [a; 4]                   % Vertical concatenation (also see vertcat)
...
% Read and try the rest of the tutorial as homework!
% You only need to know how to read, save, and convert an image to grayscale,
% for HW1P.
...
axis([0 6 0 8]);             % Adjust visible rectangle
...
plot(x, 2*cos(x), 'k^');     % '--' chooses different line pattern
plot(x, 3*cos(x), 'go');     % 'go' plots the line with g_reen o_vals
                             % see plot -> 'LineSpec' for more
legend('sin', '2cos', '3cos');      % Assigns names to each plot
...
I5 = uint8(I4);    
unique(I5(:))                 % Print unique elements to verify 
figure; imshow(I5);           % All dark! Why? 
figure; imagesc(I5);          % Same with imagesc also
I6 = 100 * I5;                % Scale pixel values
unique(I6(:))                 % Print unique elements
figure; imshow(I6);           % What happened? 
figure; imagesc(I6);  
...
figure; imagesc(I7)           % imagesc works directly on double, also note it displays in color
...
% Another example of imshow vs imagesc
I9 = rgb2gray(I);
class(I9)                     % uint8
figure; imshow(I9)            % Both functions work (differently) with uint8
figure; imagesc(I9)
I10 = double(I9);
figure; imshow(I10)
...
%% (7) Working with images 2: Most common mistakes
% This section is about indexing a matrix or an image.

% Suppose this is a 4 by 5 "image":
I = reshape(1:20, [4, 5])
% I =
%      1     5     9    13    17
%      2     6    10    14    18
...
% How do we access "7" which is in row 3, column 2?

% Option 1: I as a matrix with row and column:
row = 3;        % row is a vertical index -> 3rd row. 
col = 2;        % col is a horizontal index -> column.
I(row, col)     % I(vertically from top by "row", horizontally from left by "col")
% We treat I as a matrix and access the element by the I(row,col) convention
% where row is the VERTICAL index, and col is the HORIZONTAL index.
...
% Option 2: I as an image with x and y coordinates:
y = 3;          % y is a vertical index -> 3 pixels down.
x = 2;          % x is a horizontal index -> 2 pixels right.
I(y, x)         % I(vertically from top by "y", horizontally from left by "x")
% We treat I as an image where the upper left corner is the origin. We can
% access the pixel by I(y,x) convention where y is the VERTICAL offset from
% the top, and x is the HORIZONTAL offset from the left.
...
% Rule of thumb: Every image is a matrix, and we always access the
% pixels/elements by (row, column) convention. This is equivalent to (y, x).
% I(row, col) = I(y, x)

% Common Error 1:
% If you have been thinking about your algorithm in terms of (x,y)
% coordinates, make sure you either 
% (1) consciously do I(y,x) or
% (2) set row = y, col = x, then always stick with I(row,col) convention.

% Common Error 2:
% You want to access every pixel of an image with a nested for-loops.
% You use size(I) to get the output [4,5] and check the image size.
% It feels natural to interate through x then y this way. 
% for x=1:4
%   for y=1:5
%       I(x,y)
%   end
...
% (1) [4,5] = size(I) gives you maximum row (or x) and column (or y). Thus,
% you need to do x=1:5 and y=1:4.
% (2) I(x,y) needs to be I(y,x).
...
% What you may also do is to think everything in terms of rows and columns:
